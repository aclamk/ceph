...
extern void __ceph_assert_fail(const __ceph_assert_data &ctx)
  __attribute__ ((__noreturn__));
...
#define assert(expr) \
  do { static const __ceph_assert_data __ceph_assert_data_ctx = \
   {__STRING(expr), __FILE__, __LINE__, __CEPH_ASSERT_FUNCTION}; \
   ((expr) \
   ? _CEPH_ASSERT_VOID_CAST (0)	\
   : __ceph_assert_fail(__ceph_assert_data_ctx)); } while(false)
