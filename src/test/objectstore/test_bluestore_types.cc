...
TEST(ExtentMap, reshard_split_creates_empty_blob_right) {
  // The test is trying to trick reshard() into creating zombie spanning blob
  // BLOB 64K, 16K chsum
  // AAAA ---- ---- ----
  //     ^             ^
  //   split1       split2
  //
  // Expected: split1 creates empty blob that gets deleted.
  //           split2 does nothing
  BlueStore store(g_ceph_context, "", 4096);
  std::unique_ptr<BlueStore::OnodeCacheShard> oc(
    BlueStore::OnodeCacheShard::create(g_ceph_context, "lru", NULL));
  ...
  auto coll = ceph::make_ref<BlueStore::Collection>(&store, oc.get(), bc.get(), coll_t());
  BlueStore::Onode onode(coll.get(), ghobject_t(), "");
  // csum block size = 16K, full blob length covered with csum
  ...
  int csum_type = Checksummer::CSUM_CRC32C;
  {
    ...
    std::cout << onode.print(42) << std::endl;
    EXPECT_EQ(em.spanning_blob_map.size(), 0);
    for(auto& bi : em.spanning_blob_map) {
      EXPECT_NE(bi.second->get_blob().get_ondisk_size(), 0);
    }
  }
  EXPECT_EQ(mempool::bluestore_blob::allocated_items(), 1);
}

TEST(ExtentMap, reshard_split_creates_empty_blob_left) {
  // The test is trying to trick reshard() into creating zombie spanning blob
  // BLOB 64K, 16K chsum
  // ---- ---- AAAA ----
  //          ^   ^
  //     split1   split2
  //
  // Expected: split1 creates empty blob on the left that gets deleted.
  //           split2 creates spanning blob
  BlueStore store(g_ceph_context, "", 4096);
  std::unique_ptr<BlueStore::OnodeCacheShard> oc(
    BlueStore::OnodeCacheShard::create(g_ceph_context, "lru", NULL));
  std::unique_ptr<BlueStore::BufferCacheShard> bc(
  ...
  int csum_type = Checksummer::CSUM_CRC32C;
  {
    ...
    {
    auto& dbb = bb1->dirty_blob();
    dbb.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET,0x8000));
    dbb.allocated_test(bluestore_pextent_t(0x12345000,0x4000));
    dbb.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET,0x4000));
    dbb.init_csum(csum_type, csum_order, 0x10000);
    em.set_lextent(coll, 0x18000, 0x8000, 0x4000, bb1, nullptr);
    }
    std::cout << "start:" << std::endl;
    std::cout << onode.print(42) << std::endl;

    BlueStore::ExtentMap::ReshardPlan rp;
    rp.new_shard_info.emplace_back(0x18000, 0);
    rp.new_shard_info.emplace_back(0x1b000, 0);
    rp.shard_index_begin = 0;
    rp.shard_index_end = 1;
    rp.spanning_scan_begin = 0; // doesn't matter
    rp.spanning_scan_end =  0; // doesn't matter

    em.request_reshard(0x10000, 0x20000);
    em.reshard_action(rp, nullptr, nullptr);
    std::cout << "resharded:" << std::endl;
    std::cout << onode.print(42) << std::endl;
    EXPECT_EQ(em.spanning_blob_map.size(), 1);
    for(auto& bi : em.spanning_blob_map) {
      EXPECT_NE(bi.second->get_blob().get_ondisk_size(), 0);
    }
  }
...
TEST(ExtentMap, reshard_split_creates_empty_blob_center) {
  // The test is trying to trick reshard() into creating zombie spanning blob
  // BLOB 64K, 16K chsum
  // AAAA ---- ---- BBBB
  //     ^         ^   ^
  ...
  //
  // Expected: split1 creates 2 non-empty blobs.
  //           split2 creates empty blob on the left that gets deleted.
  //           split3 creates spanning blob
  BlueStore store(g_ceph_context, "", 4096);
  std::unique_ptr<BlueStore::OnodeCacheShard> oc(
    BlueStore::OnodeCacheShard::create(g_ceph_context, "lru", NULL));
  std::unique_ptr<BlueStore::BufferCacheShard> bc(
  ...
  int csum_type = Checksummer::CSUM_CRC32C;
  {
    ...
    dbb.allocated_test(bluestore_pextent_t(0x12345000,0x4000));
    dbb.allocated_test(bluestore_pextent_t(bluestore_pextent_t::INVALID_OFFSET,0x8000));
    dbb.allocated_test(bluestore_pextent_t(0xabcde000,0x4000));
    dbb.init_csum(csum_type, csum_order, 0x10000);
    em.set_lextent(coll, 0x10000, 0x0, 0x4000, bb1, nullptr);
    em.set_lextent(coll, 0x1c000, 0xc000, 0x4000, bb1, nullptr);
    }
    std::cout << "start:" << std::endl;
    std::cout << onode.print(42) << std::endl;

    BlueStore::ExtentMap::ReshardPlan rp;
    rp.new_shard_info.emplace_back(0x14000, 0);
    rp.new_shard_info.emplace_back(0x1c000, 0);
    rp.new_shard_info.emplace_back(0x1f000, 0);
    rp.shard_index_begin = 0;
    rp.shard_index_end = 2;
    rp.spanning_scan_begin = 0; // doesn't matter
    rp.spanning_scan_end =  0; // doesn't matter
  ...
  EXPECT_EQ(mempool::bluestore_blob::allocated_items(), 2);
}
