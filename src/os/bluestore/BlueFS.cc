...
int BlueFS::flush_and_sync_log(uint64_t want_seq)
{
  ...
  //log_writer->lock.lock();
  
  //std::unique_lock<ceph::mutex> ll(log.writer->file->lock);
  ...
  ceph_assert(want_seq == 0 || want_seq <= dirty.seq_next); // illegal to request seq that was not created yet
