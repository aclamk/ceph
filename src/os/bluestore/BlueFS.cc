...
// 1) lock buf
// 2) copy as much as we can from buf
// 3) unlock buf
// 4) return if satisfied
// 5) calculate how many bytes we want to read
// 6) lock file
// 7) copy relevant extents from file->fnode
// 8) unlock file
// 9) read data into temp buf
// 10) copy data to out
// 11) lock buf
// 12) move remaining data from temp buf to buf
// 13) unlock buf
...
uint64_t BlueFS::estimate_log_size_N()
{
...
void BlueFS::compact_log()
{
  if (!cct->_conf->bluefs_replay_recovery_disable_compact) {
    if (cct->_conf->bluefs_compact_log_sync) {
      ...
      _compact_log_async_LD_N_D();
    }
  }
}

bool BlueFS::should_start_compact_log_L_N()
{
  ...
  uint64_t expected = estimate_log_size_N();
...
void BlueFS::compact_log_dump_metadata_N(bluefs_transaction_t *t,
					int flags)
{
  std::lock_guard nl(nodes.lock);
					 ...
					 std::optional<bluefs_layout_t> layout)
{
  ...
  dout(20) << __func__ << " super_dev:" << super_dev
		       ...
		       << dendl;
  bluefs_transaction_t t;
  compact_log_dump_metadata_N(&t, flags);
...
{
  // 0. under log.lock, so no new log.t elements can be added
...
void BlueFS::_compact_log_async_LD_N_D() //also locks FW for new_writer
{
  dout(10) << __func__ << dendl;
  ...
  bluefs_transaction_t t;
  //avoid record two times in log.t and _compact_log_dump_metadata.
  //AK - seems redundant:
  //log.t.clear();
  ...
  // we might have some more ops in log.t due to _allocate call, ak:really?
  //t.claim_ops(log.t);
...
// 1. log lock - protects any operation on bluefs log, including adding to log.t
// 2. lock for dirty.files
// 3. compaction_lock - locks activities relating to compaction
// what lock is needed for _flush_and_sync_log ?
// it needs some lock to execute exclusively
...
void BlueFS::clear_dirty_set_stable_D(uint64_t seq)
{
...
/*
  0) called with filewriter lock taken
  1) lock file
  2) calculate needed space
  3) maybe allocate
  4) prepare extents to write to
     add file to dirty list
  5) unlock file
  6) prepare data 
  7) maybe wait for completing previous aio
  8) write/initiate write to disk
 */
int BlueFS::_flush_range_F(FileWriter *h, uint64_t offset, uint64_t length)
{
  ...
  ceph_assert(h->file->num_readers.load() == 0);//todo really?
...
void BlueFS::_maybe_compact_log_LN_N_LD_D()
{
  if (!cct->_conf->bluefs_replay_recovery_disable_compact &&
      should_start_compact_log_L_N()) {
    if (cct->_conf->bluefs_compact_log_sync) {
      _compact_log_sync_LN_LD();
    } else {
      _compact_log_async_LD_N_D();
