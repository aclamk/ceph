...
void BlueFS::compact_log_async()
{
  ...
  // we might have some more ops in log_t due to _allocate call, ak:really?
  //t.claim_ops(log_t);
...
// 1. log lock - protects any operation on bluefs log, including adding to log_t
// 2. lock for dirty_files
...
int BlueFS::flush_and_sync_log(uint64_t want_seq)
{
  ...
  //std::unique_lock<ceph::mutex> ll(log_writer->file->lock);
