				       ...
				       std::string* _fn, std::string* _kv_backend)
{
  ...
  // simplify the dir names, to ones as "seen" by rocksdb
  fn = "db";
  kv_backend = "rocksdb";
  if (create) {
    // create also opens
...
/**
   Reads setting S.per_pool_omap from db
...
int BlueStore::mkfs()
{
  ...
  // we set ondisk_format to both DB and label.meta
...
/**
   There are 2 ondisk_format values; in label.meta and in DB.
...
/**
  With ondisk_format = 5, all settings are moved from DB to label.meta.
  When label.meta "ondisk_format" does not exist, upgrade process is triggered.
 */
int BlueStore::_open_bdev_label_meta()
{
  uint64_t val;
  ...
  if (r == 0) {
    ...
    // relocate successful, open using label.meta
  } else {
    derr << __func__ << " problem reading ondisk_format" << dendl;
    return r;
  }
  r = _upgrade_bdev_label_meta();
  if (r != 0) {
    return r;
  }
  r = _read_bdev_label_meta();
  if (r != 0) {
    return r;
  }
  return 0;
...
/**
  When label.meta "ondisk_format" does not exist, upgrade process is triggered.
  Upgrade copies all settings from DB to label.meta.
  DB is opened in read-only mode; settings in DB are not deleted.
  The last modified label.meta is "ondisk_format = 5".

  Just after upgrade label.meta.ondisk_format == 5, DB's ondisk_format is still 4.
...
int BlueStore::_relocate_db_settings_to_super_meta()
{
  ...
  for (auto& k : keys_to_move) {
	...
	if (r != 0) {
	  derr << __func__ << " error writing label.meta " << k.name << "=" << bl.to_str() << dendl;
	  result = -EIO;
	  break;
	}
      } else {
	...
	if (r != 0) {
	  derr << __func__ << " error writing label.meta " << k.name << "=" << std::to_string(v) << dendl;
	  result = -EIO;
	  break;
	}
      }
 ...
 */
int BlueStore::_read_bdev_label_meta()
{
  string s;
...
// Settings related to BlueStore environment are moved to bdev_label.meta:
...
// upgrades values stored in bdev_label.meta
int BlueStore::_upgrade_bdev_label_meta()
{
...
int BlueStore::_init_bdev_label_meta()
{
  dout(10) << __func__ << " ondisk_format " << ondisk_format
	   << " min_compat_ondisk_format " << min_compat_ondisk_format
	   << dendl;
