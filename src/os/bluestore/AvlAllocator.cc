 ...
 */
template<class Tree>
uint64_t AvlAllocator::_block_picker(const Tree& t, uint64_t *cursor, uint64_t size, uint64_t align)
{
  for (auto rs = t.lower_bound(range_seg_t{*cursor, size});
       rs != t.end(); ++rs) {
...
void AvlAllocator::_add_to_tree(uint64_t start, uint64_t size)
{
  ...
  if (rs != range_tree.begin()) {
    rs_before = std::prev(rs);
  }
  auto rs_after = range_tree.end();
  if (rs != range_tree.end()) {
    rs_after = std::next(rs);
  }
  ...
  if (merge_before && merge_after) {
    ...
    rs = range_tree.insert_unique(rs, *new_rs);
  }

  assert(rs != range_tree.end());
  range_size_tree.insert_equal(*rs);
...
void AvlAllocator::_remove_from_tree(uint64_t start, uint64_t size)
{
  ...
  auto rs = range_tree.find(range_seg_t{start, end});
  ...
  if (left_over && right_over) {
    ...
    rs->end = start;
    range_tree.insert_unique(rs, *new_seg);
    range_size_tree.insert_equal(*new_seg);
  } else if (left_over) {
    rs->end = start;
  } else if (right_over) {
    rs->start = end;
...
{
  std::lock_guard l(lock);
  if (p2align(num_free, block_size) == 0) {
    return .0;
  }
  uint64_t fragments = 0;
  for (const auto& rs : range_tree) {
    auto start = p2roundup(rs.start, block_size);
    auto end = p2align(rs.end, block_size);
    auto fragment = (rs.end - rs.start);
    if (start < end) {
      fragment -= (end - start);
    }
    fragments += fragment;
  }
  return (double)fragments / p2align(num_free, block_size);
}
