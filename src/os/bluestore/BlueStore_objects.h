...
protected:
  void* do_allocate(size_t bytes, size_t alignment) override {
    ...
    if (aligned_offset > current_slab->offset) {
      size_t gap_size = aligned_offset - current_slab->offset;
      void* gap_ptr = static_cast<char*>(current_slab->data()) + current_slab->offset;
      size_freelist.insert(FreeBlock{gap_ptr, gap_size});
      ptr_map[gap_ptr] = gap_size;
    }
  ...
  void do_deallocate(void* p, size_t bytes, size_t) override {
    ...
    if (ptr_map.find(p) != ptr_map.end()) throw std::runtime_error("Double deallocation detected");
